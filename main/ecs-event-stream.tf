...
resource "aws_cloudwatch_event_rule" "ecs_event_stream" {
  name        = "${var.ecs_cluster_name}-ecs-event-stream"
  description = "Passes ecs event logs for ${var.ecs_cluster_name} to a lambda that writes them to cw logs"
...
resource "aws_lambda_function" "ecs_event_stream" {
  function_name    = "${var.ecs_cluster_name}-ecs-event-stream"
...
resource "aws_cloudwatch_dashboard" "ecs-event-stream" {
  dashboard_name = "${var.ecs_cluster_name}-ecs-event-stream"
  ...
  "widgets": [
    {
      ...
      "height": 18,
      "properties": {
        "query": "SOURCE '/aws/lambda/${var.ecs_cluster_name}-ecs-event-stream' | fields @timestamp as time, detail.desiredStatus as desired, detail.lastStatus as latest, detail.stoppedReason as reason, detail.containers.0.reason as container_reason, detail.taskDefinitionArn as task_definition\n| filter @type != \"START\" and @type != \"END\" and @type != \"REPORT\"\n| sort detail.updatedAt desc, detail.version desc\n| limit 100",
        "region": "${var.region}",
        "title": "ECS Event Log"
      }
    }
